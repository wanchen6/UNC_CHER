...
def read_content(csv_content):
    ...
    for row in csv_content:
        ...
        if category_content == 'Event':
        ...
        elif category_content == 'Associates' and ('Giselle' not in title_content and 'Giselle' not in final_txt_content):
            ...
            if mydate < date.today() - timedelta(days=16):
                ...
                # Combine events together
                news_html = news_html + div_top_html + title_html + date_html + location_award_html + abstract_html + div_down_html
                news_html_wp = news_html_wp + div_top_html + title_html + date_html + location_award_html + abstract_html + div_down_html

        elif category_content == 'Associates' and ('Giselle' in title_content or 'Giselle' in final_txt_content):
            mydate = datetime.strptime(post_date_content, '%m/%d/%y').date()
            if mydate < date.today() - timedelta(days=16):
                # break current loop
                continue
                ...
                # Combine events together
                ldr_html = ldr_html + div_top_html + title_html + date_html + location_award_html + abstract_html + div_down_html
                ldr_html_wp = ldr_html_wp + div_top_html + title_html + date_html + location_award_html + abstract_html + div_down_html
            ...
            if mydate < date.today() - timedelta(days=16):
                ...
                # Combine events together
                pub_html = pub_html + div_top_html + title_html + date_html + location_award_html + abstract_html + div_down_html
                pub_html_wp = pub_html_wp + div_top_html + title_html + date_html + location_award_html + abstract_html + div_down_html
    ...
    associate_html = associate_html + wip_html + news_html + pub_html + ldr_html + '\n\t\t</section>'
    associate_html_wp = associate_html_wp + wip_html_wp + news_html_wp + pub_html_wp + ldr_html_wp + '[/tab]'
