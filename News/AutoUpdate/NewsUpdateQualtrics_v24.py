...
class htmlGenerater:
    ...
    def create_html(self):
        ...
        # Call read_content
        try:
            with open(filename, 'r', encoding='utf-8-sig', newline='', errors='ignore') as csvfile:
    ...
    def sortEvent(self, csv_content):
        #sort by category, event_start, event_end
        csv_content_filter_event = filter(lambda x: x['V18'] == 'CHER Event' or x['V18'] == 'External Event', csv_content)
        csv_content_filter_event = sorted(csv_content_filter_event, key=itemgetter('V18'))
        #sort by start date and end date
        csv_content_filter_event = sorted(csv_content_filter_event, key=lambda x: (self.try_parsing_date(x['V21']).date(), self.try_parsing_date(x['V22']).date()))
        return csv_content_filter_event

    # Sort the csv file by category
    def sortNews(self, csv_content):
        #sort by category, post_date
        csv_content_filter_news = filter(lambda x: x['V18'] == 'CHER News' or x['V18'] == 'CHER Publication', csv_content)
        csv_content_filter_news = sorted(csv_content_filter_news, key=itemgetter('V18'))
        csv_content_filter_news = sorted(csv_content_filter_news, key=lambda x: self.try_parsing_date(x['V17']).date(), reverse=True)
        return csv_content_filter_news

    # Sort the csv file by category
    def sortFundingAbstract(self, csv_content):
        #sort by category, institution, dead_date
        csv_content_filter_fa = filter(lambda x: x['V18'] == 'Funding Abstract', csv_content)
        csv_content_filter_fa = sorted(csv_content_filter_fa, key=itemgetter('V12'))
        csv_content_filter_fa = sorted(csv_content_filter_fa, key=lambda x: self.try_parsing_date(x['V26']).date())
        return csv_content_filter_fa

    # Sort the csv file by category
    def sortFunding(self, csv_content):
        # sort by category, dead_date
        csv_content_filter_f = filter(lambda x: x['V18'] == 'Funding', csv_content)
        csv_content_filter_f = sorted(csv_content_filter_f, key=lambda x: self.try_parsing_date(x['V26']).date())
    ...
    def read_content(self, csv_content, inst, writer):
        ...
        # Read CSV
        for row in csv_content:
            id_content = row['V1'] #ResponseID
            try:
                headline_content = row['headline']
            except:
                headline_content = ''
            title_content = row['V15'] #title
            if row['V12'] == 'Other institution not in the list': #institution
                institution_content = row['V13'] #institution_info-Institution Name
                inst_url_content = row['V14']#institution_info-URL
            else:
                institution_content = row['V12']#institution
                inst_url_content = inst[institution_content]
            location_content = row['V23'].replace('/ ', '<br>')#location
            post_date_content = row['V17']#post_date
            dead_url_content = row['V27']#dead_url
            dead_date_content = row['V26']#dead_date
            apply_date_content = row['V25']#apply_date
            event_start_content = row['V21']#event_start
            event_end_content = row['V22']#event_end
            post_url_content = row['V16']#post_url
            category_content = row['V18']#category
            award_amt_content = row['V28']#award_amt
            img_url_content = row['V32']#img_url
            final_txt_content = row['V31'].replace('/ ', '<br>')#final_txt
            ...
            if category_content == 'External Event':
                ...
                if event_start_date < date.today() and headline_content != 'Y':
                    ...
                    # Add title
                    title_html_wp = '\n\t\t\t\t<h1 class="title" style="font-weight: 400;margin: 0.5em 0 0.5em 0; clear: both; font-size:1.5em;line-height:1.5em;"><a target="_blank" href="' + post_url_content + '" style="text-decoration: none; color: #56a0d3; ">' + title_content + '</a></h1>'
                    ...
                    location_content = html.unescape(location_content)
                    location_award_html = '\n\t\t\t\t\t\t\t\t<h3 class="location-award" style="line-height: 1.3em; font-weight: 400; margin: 0 0 0.5em 0; font-size: 1.1em;">' + location_content + '</h3>'
                ...
                if final_txt_content != '':
                    ...
                    # Add location or award
                    location_award_html = '\n\t\t\t\t<h3 class="location-award" style="line-height: 1.3em;font-weight: 400; margin: 0 0 0.5em 0; font-size: 1.1em;">Funding: ' + award_amt_content + '</h3>'
                ...
                if funding_abstract_counter > 2 or post_date < date.today() - timedelta(days=45):
                    # break current loop
                    continue
                ...
                if institution_content == funding_inst:
                    ...
                    # Add title
                    title_html = '\n\t\t\t\t<h1 class="title" style="font-weight: 400;margin: 0.5em 0 0.5em 0; clear: both; font-size:1.5em;line-height:1.5em;"><a target="_blank" href="' + inst_url_content + '" style="text-decoration: none; color: #56a0d3; ">' + institution_content + '</a></h1>'
                ...
                if post_date < date.today() - timedelta(days=16) and headline_content != 'Y':
                    ...
                    # Add location or award
                    location_award_html = '\n\t\t\t\t<h3 class="location-award" style="line-height: 1.3em;font-weight: 400;margin: 0 0 0.5em 0; font-size: 1.1em;">Institution: <a target="_blank" href="' + inst_url_content + '" style="text-decoration: none; color: #56a0d3;">' + institution_content + '</a></h3>'
        ...
        if news_html_headline+news_html == '<h1 style="font-weight: 600; margin: 0.5em 0 0.5em 0; font-size:1.5em; color: #E4A552;">CHER News</h1>':
            news_html_headline = ''
        if news_html_wp_headline+news_html == '<h1 style="font-weight: 600; margin: 0.5em 0 0.5em 0; font-size:1.5em; color: #E4A552;">CHER News</h1>':
        ...
        content_html_WP = '[tabs class="cher-newsletter"]' + associate_html_wp + event_html_wp + funding_html_wp + '''[/tabs]\n<div style="margin-left: 20px">[button style="btn-primary btn-lg" icon="fa fa-newspaper-o" align="left" type="link" target="true" title="Past CHER News" link="https://cher.unc.edu/cher-news-events/" linkrel=""]  [button style="btn-primary btn-lg" icon="fa fa-envelope-o" align="left" type="link" target="true" title="Subscribe to CHER Newsletter" link="http://eepurl.com/cI92vT" linkrel=""]  [button style="btn-primary btn-lg" icon="fa fa-search" align="left" type="link" target="true" title="Search Publications" link="https://cher.unc.edu/publication-search/" linkrel=""]</div>'''
